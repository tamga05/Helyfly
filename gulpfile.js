...
//const ghPages = require('gulp-gh-pages'); // для Создания Сайта на gh-pages и "Деплоя" на этот сайт моего проекта (для деплоя-раскомментировать строку 20).
...
// таска для Создания Сайта на gh-pages и "Деплоя" на этот сайт моего проекта (для деплоя-раскомментировать строки 65-68).
...
// 2. B консоли VSCode открыть Папку с Новым проектом, написать и запустить команду npm install (установятся все нужные модули);

// 3. После того, как всё корректно установится (в браузере Откроется приветственная Страничка "Hello, Andrey") и в папке с Новым проектом появятся папки dist, node_modules, package-lock.json, нужно ОБЯЗАТЕЛЬНО Удалить из этой папки с Новым проектом Пустые файлы index.html и style.css (которые в самом начале перенесли из Папки my-gulp-start в эту папку с Новым проектом) и из папки src/assets/scss Удалить папку _style2.scss !!! и в папке main.scss Удалить строчку с кодом @import "./style2" !!!;

// 4. Соблюдаем файловую структуру или в сборках подправляем пути "откуда берем"/"куда кладем" файлы.

//5. !!!!! Для Деплоя готового проекта на GitHub-pages Раскомментировать в gulpfile.js строки 20, 65-68 и в консоли VSCode
//   ввести сначала команду npm install--save - dev gulp - gh - pages после её отработки ввести команду gulp deploy   !!!!!